...
from coverage5.backward import StringIO, import_local_file
from coverage5.control import Plugins
...
class LoadPluginsTest(CoverageTest):
    ...
    def test_cant_import(self):
        with self.assertRaisesRegex(
            ImportError, "No module named '?plugin_not_there'?"
        ):
...
class PluginTest(CoverageTest):
    ...
    def test_missing_plugin_raises_import_error(self):
        ...
        with self.assertRaisesRegex(
            ImportError, "No module named '?does_not_exist_woijwoicweo'?"
        ):
            cov = coverage.Coverage()
    ...
    def test_plugin_sys_info(self):
        ...
        if env.C_TRACER:
            ...
            self.assertIn(
                "plugins.file_tracers: plugin_sys_info.Plugin (disabled)", out_lines
            )
...
class GoodFileTracerTest(FileTracerTest):
    ...
    def test_plugin2_with_text_report(self):
        ...
        repout = StringIO()
        total = cov.report(
            file=repout, include=["*.html"], omit=["uni*.html"], show_missing=True
        )
...
class BadFileTracerTest(FileTracerTest):
    ...
    def run_bad_plugin(
        self, module_name, plugin_name, our_error=True, excmsg=None, excmsgs=None
    ):
        ...
        if excmsgs:
            self.assertTrue(
                any(em in stderr for em in excmsgs), "expected one of %r" % excmsgs
            )
