...
from coverage5.control import Coverage
from coverage5.data import CoverageData
from coverage5.misc import CoverageException, output_encoding
...
class SummaryTest(UsingModulesMixin, CoverageTest):
    ...
    def test_report_show_missing(self):
        ...
        self.assertEqual(
            self.last_line_squeezed(report), "mymissing.py 14 3 79% 3-4, 10"
        )
    ...
    def test_report_show_missing_branches(self):
        ...
        self.assertIn("mybranch.py ", report)
        self.assertEqual(
            self.last_line_squeezed(report), "mybranch.py 6 0 4 2 80% 2->4, 4->exit"
        )
    ...
    def test_report_skip_covered_longfilename(self):
        ...
        out = self.run_command(
            "coverage run --source=. --branch long_______________filename.py"
        )
        self.assertEqual(out, "")
        report = self.report_from_command("coverage report --skip-covered")
