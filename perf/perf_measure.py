...
class StressResult(
    namedtuple("StressResult", ["files", "calls", "lines", "baseline", "covered"])
):
    @property
    def overhead(self):
...
def mk_main(file_count, call_count, line_count):
    ...
    lines.extend(
        "test{}.parent({}, {})".format(idx, call_count, line_count)
...
class StressTest(object):
    ...
    def _compute_overhead(self, file_count, call_count, line_count):
        baseline, covered, stats = self._run_scenario(
            file_count, call_count, line_count
        )
